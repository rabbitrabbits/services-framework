 ...
 **/
public class StepModelCSV {
    public static final Logger logger = LoggerFactory.getLogger(StepModelCSV.class);
    ...
    public StepResult run(HashMap<String, String> testCaseVariables){
        ...
        if (asserts != null) {
            ...
            asserts.forEach(assertModel -> {
                ...
                finalAssertModel.setExpect( PlaceholderUtils.resolveString(assertModel.getExpect(),testCaseVariables));
               assertList.add(()->{
                   //利用反射获取需要断言的方式
                   Class<Matchers> matchersClazz = (Class<Matchers>) Class.forName("org.hamcrest.Matchers");
                   Method matherMethod = matchersClazz.getDeclaredMethod(finalAssertModel.getMatcher(), Object.class);
                   Matcher invoke = (Matcher)matherMethod.invoke(matchersClazz.newInstance(), finalAssertModel.getExpect());

                   //断言
                   assertThat(finalAssertModel.getReason(),response.path(finalAssertModel.getActual()).toString(),invoke);
               });
            });
        }
        /**
